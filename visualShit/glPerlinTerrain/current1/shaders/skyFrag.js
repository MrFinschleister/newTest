const skyFragmentShader = `
    ...
    void main(void) {
        ...
        vec2 uvCloud = normalize(vPosition).xy * 0.5 + 0.5;

        float percentOfDay = mod(uTimeOfDay / uTimePerDay, 1.0);
        float directionDot = dot(direction, vec3(0.0, 1.0, 0.0));

        float cloudValue = getCloudValue(uvCloud, directionDot, uCloudContrast, uCloudExponent, uCloudBias, uCloudSpeed, uTick, uCloudFrequency, uCloudFrequencyCoeff, uCloudAmplitude, uCloudAmplitudeCoeff);
